...
首先，让我们研究一个完全没有任何知识或信息的人参加考试时的期望结果。在这种信息缺乏的情况下，即使是一个完全理性的贝叶斯考生，需要把所有选项视为等概率，最后变成一个随机作答者。
...
现在我们可以提出一个更有用的问题：如果 $p$ 提高1个百分点，我们期望多得多少分？对于每个 $p$，这可以计算为 `expected_score(p + 0.01) - expected_score(p)`。TF和C15的敏感度是平坦的，而C25表现出非线性行为。
...
我们看到，即使游戏是公平的，有些宏观状态也会更可能出现，因为它们更“模糊”：它们对应更多微观状态。而且玩得越多，这种现象越夸张。让我们把游戏的名义金额 $N$ 定义为最大可能输赢金额。现在比较20次、400次和2000次游戏之后，结果落在净收益零附近的可能性。分布会越来越集中在零附近。这在理论上很直观：游戏次数越多，零附近微观状态组合的数量增长得比其他区域快得多。读者可以自行研究数学细节，推导上一节中大量使用的二项分布，并得出结论：分布宽度以指数为1/2的多项式速度收缩。
